...
resources/js/pages/home.tsx              **HP en ordre de conversion** (décision utilisateur 2026-09-22) : `Hero` (bouton plein → contact) → `TrustIntro` (chiffre réel `about.stats.2`, conseillers, note Google, 2 CTA) → `Services` → `AgencyManifesto` « Qui sommes-nous ? » en texte seul (prop `stats` omise, pas de doublon avec le chiffre du bloc de confiance) → `SuccessStories` (`quote={false}` : carrousel seul) → `Testimonials` → `FaqTeaser` thème « Travailler avec Estate in Paris » (`home.faq.*`, prop `faq` via `ExcerptFaqCategory('working', 6)`, JSON-LD `faqPage` + `siteGraph`) → `BlogRelated` (3 derniers articles Sanity, masqué si Sanity échoue) → `CtaCard` (bouton plein). Chaque h2 est une question que le bloc suivant résout ; un seul bouton plein en haut et en bas. `HomeTest` → décisions et détails : docs/decisions.md
