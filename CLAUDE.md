...
- **Variables d'environnement** à renseigner dans Cloud (valeurs réelles, aucun placeholder) : `APP_URL` (https, www ou non selon le domaine canonique), `APP_ENV=production`, `APP_DEBUG=false`, `INERTIA_SSR_ENABLED`, tout le bloc `SEO_*` (org, téléphone, adresse, réseaux, horaires, avis), `MAIL_*`, `GTM_ID`.
