...
resources/js/pages/home.tsx              home = en-tête standard (`<PageEyebrow>` `home.title` + h1 `home.headline` + intro `home.intro`, centré, `max-w-3xl`) ; le hero photo (Figma 123-304, `components/home/hero.tsx`, `public/images/home/hero-*.jpg`, prop `hero` de `PublicLayout`) a été **supprimé le 2026-09-16** (décision utilisateur) ; **intro de confiance** `components/home/trust-intro.tsx` sous le hero (chiffre réel `about.stats` + avis Google + 2 CTA, prop `figure`, voir `docs/decisions.md`) ; test `HomeTest`
