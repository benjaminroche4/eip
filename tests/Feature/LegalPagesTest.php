...
class LegalPagesTest extends TestCase
{
    ...
    {
        $this->withLocale('fr')->get('/fr/mentions-legales')->assertOk()->assertInertia(fn (Assert $page) => $page->component('legal')->where('page.key', 'legal')->where('page.title', 'Mentions légales'));
    ...
    public function test_legal_pages_are_listed_in_the_sitemap(): void
    {
        ...
        $this->assertStringContainsString('<loc>'.url('/fr/mentions-legales').'</loc>', $xml);
    ...
    public function test_footer_trust_data_is_shared(): void
    {
        ...
        $this->withLocale('fr')->get('/fr')->assertInertia(fn (Assert $page) => $page->where('seo.reviews.count', 128)->where('seo.hours.label', config('seo.hours.labels.fr')));
    }
}
