 ...
 * on the site's sand band (the Figma's grey `#F8F8F8` becomes `from-background-05 to-background`, breaking out to the
 * full screen width, exactly like the testimonials). Centred header (eyebrow, h2 as a question, answer-first intro),
 * then four cards in the site's card (sand hairline, `p-2`, inner sand gradient, square corners, no shadow): a lucide
 * icon in a square tile and a sand Montserrat number `01`-`04` on the top line (same numbering as the About values), a
 * large sand watermark of the icon in the corner (as on the home services), the title (`text-base`) and one factual sentence (`text-sm/6` at every width, smaller than the site's
 * paragraphs on user request 2026-09-21) at the bottom. On hover the card takes the services' look (darker hairline, dark icon tile, stronger watermark — colours only,
 * purely decorative since the card is not a link). The cards rise in cascade when the grid enters the viewport (`animate-hero-rise`, `--stagger` 80 ms,
 * `motion-reduce` cancels). Stacked on mobile like the Figma, 2×2 from `sm`, four columns from `lg`. The only
 * interactive element is the outline button to contact under the grid — improvements applied on user decision 2026-09-21.
 */
export default function BuyAdvantages() {
    ...
    return (
        <section
            aria-labelledby="buy-advantages-title"
        ...
        >
            <div className="mx-auto flex max-w-7xl flex-col gap-10 px-6 py-16 sm:py-20 lg:gap-14 lg:px-8">
                <div className="mx-auto flex max-w-3xl flex-col items-center gap-4 text-center">
                    <PageEyebrow>{t('buy.why_eyebrow')}</PageEyebrow>
                    <h2 id="buy-advantages-title" className="text-2xl font-medium tracking-tight text-balance sm:text-3xl">
                        {t('buy.why_title')}
                    </h2>
                    {/* GEO: a self-contained sentence (brand + what + where) */}
                    <p className="text-muted-foreground max-w-2xl text-base/7 text-pretty sm:text-sm/6">{t('buy.why_intro')}</p>
                </div>
                        ...
                        >
                            <div className="from-background-05 relative flex min-h-56 w-full flex-col justify-between gap-10 overflow-hidden bg-linear-to-b to-transparent p-6 lg:min-h-72">
                                ...
                                <div className="relative flex flex-col gap-3">
                                    <h3 className="text-base font-medium text-balance">{t(`buy.why_${key}_title`)}</h3>
                                    <p className="text-muted-foreground text-sm/6 text-pretty">{t(`buy.why_${key}_text`)}</p>
                                </div>
                            </div>
                        </li>
                    ))}
                ...
                {/* A way out of the reassurance block: talk to an advisor */}
